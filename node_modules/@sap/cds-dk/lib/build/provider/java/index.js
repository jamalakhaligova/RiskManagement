    ...
    async build() {
        ...
        if (!this.hasBuildOption(CONTENT_EDMX, false)) {
            ...
            if (!this.hasBuildOption(CONTENT_LOCALIZED_EDMX, false)) {
                await this.compileToEdmx(odata, compileDest, { [FLAVOR_LOCALIZED_EDMX]: true })
            }
            // non-localized edmx
            await this.compileToEdmx(odata, path.join(compileDest, 'odata', cds.env.odata.version), { [FLAVOR_LOCALIZED_EDMX]: this.hasBuildOption(FLAVOR_LOCALIZED_EDMX, true) })
        }
